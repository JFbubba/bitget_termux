...
## ERR-023 · 2026-07-22 · Recenser les consommateurs d'un module par grep STATIQUE seul (l'import dynamique est invisible)

**Contexte.** Avant le refactor decision_core (extraction du cœur commun des 4 CLIs
d'analyse de 1re génération), le recensement des consommateurs a été fait par
`grep "import position_sizer\|from position_sizer …"` sur tout le dépôt → zéro
résultat → conclusion « aucun consommateur, refactor à faible risque ». FAUX :
`tests_audit.test_indicator_functions_are_centralized` consomme ces modules via
`importlib.import_module(m)` avec le nom en CHAÎNE — invisible à un grep d'énoncés
d'import. La porte 1 est passée ROUGE (725/726, `AttributeError: module
'position_sizer' has no attribute 'ema'`) au premier `bash gates.sh` : aucun dégât
(la chaîne `&&` a stoppé le commit), mais l'affirmation « aucun consommateur » avait
déjà été énoncée au propriétaire sur une preuve incomplète.
**Cause racine.** Un grep d'énoncés d'import ne voit que les DÉPENDANCES STATIQUES.
Les consommateurs dynamiques — `importlib.import_module("x")`, `__import__("x")`,
`getattr(mod, name)` piloté par une table, un nom de module en chaîne dans une liste
de config/test — référencent le module par DONNÉE, pas par syntaxe. Or c'est
précisément le pattern des gardes génériques (tests paramétrés par listes de modules,
audits de câblage) : là où un refactor d'attributs de module casse le plus.
**Solution.** Le recensement de consommateurs avant refactor/renommage cherche le nom
du module comme CHAÎNE, pas seulement comme import : `grep -rn "nom_module"` (nu,
entre guillemets, dans les .py ET les tests/configs), puis inspection des hits
`import_module`/`__import__`/tables de noms. Et surtout : les portes AVANT d'énoncer
un verdict de risque — c'est le passage de `gates.sh` qui a attrapé l'oubli, pas la
relecture.
**Contrôle (détection ailleurs).** Avant tout refactor qui déplace/supprime des
attributs d'un module M : `grep -rn "\"M\"\|'M'" --include="*.py"` (nom en chaîne) en
PLUS du grep d'imports ; vérifier spécifiquement `tests_audit.py` (gardes paramétrées
par listes de modules — ex. `test_indicator_functions_are_centralized`,
carte `expected`) et `wiring_audit`. Un module « sans consommateur » n'existe qu'après
